...
NewsletterSubscriberSchema.virtual('engagementScore').get(function () {
    ...
    const openRate = parseFloat(this.openRate);
    const clickRate = parseFloat(this.clickRate);
...
NewsletterSubscriberSchema.statics.findByInterest = function (interest) {
    return this.find({
        interests: interest,
        status: 'active'
    }).sort({ subscribedAt: -1 });
};
NewsletterSubscriberSchema.statics.findBySource = function (source) {
